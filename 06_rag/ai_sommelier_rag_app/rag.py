...
# .env 읽기 함수
# def _load_project_environment(dotenv_path: str) -> None:
...
CHAT_MODEL_NAME = os.getenv(
    "OPENAI_CHAT_MODEL", 'gpt-5.6-luna').strip() or 'gpt-5.6-luna'

# 임베딩 모델 명
EMBEDDING_MODEL_NAME = os.getenv(
    "OPENAI_EMBEDDING_MODEL", 'text-embedding-3-small') or 'text-embedding-3-small'

# Vector DB의 차원 수
EMBEDDING_DIMENSIONS = 1536

# 파인콘 인덱스 이름
PINECONE_INDEX_NAME = os.getenv("SOMMELIER_PINECONE_INDEX") or 'sommelier-index'
...
def search_wines(dish_flavor: str) -> dict[str, str]:
    ...
    # - 지정된 index_name, namespace가 같은 곳과 연결
    ...
    # 최종 답변 생성 LLM(== Generation)에 전달할 dict(==query)
    return {
...
def recommend_wines(payload: dict[str, str]) -> Runnable:
    ...
    # dict -> PromptValue -> AIMessage -> str(한국어 와인 추천)
    return prompt | _create_chat_model() | StrOutputParser()
